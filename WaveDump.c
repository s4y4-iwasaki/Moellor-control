...
int WriteOutputFiles1(WaveDumpConfig_t *WDcfg, WaveDumpRun_t *WDrun, CAEN_DGTZ_EventInfo_t *EventInfo, void *Event)
{
    int ch, j, ns;
    CAEN_DGTZ_UINT16_EVENT_t  *Event16 = NULL;
    CAEN_DGTZ_UINT8_EVENT_t   *Event8 = NULL;
    ...
    for (ch = 0; ch < WDcfg->Nch; ch++) {
        ...
        if( WDcfg->OutFileFlags& OFF_BINARY) {
            ...
            if (!WDrun->fout[ch+2]) {
                char fname[100];
                sprintf(fname, "anti%d.txt", ch);
                if ((WDrun->fout[ch+2] = fopen(fname, "w")) == NULL)
                    return -1;
            }
            if( WDcfg->OutFileFlags & OFF_HEADER) {
                // Write the Channel Header
                fprintf(WDrun->fout[ch+2], "Record Length: %d\n", Size);
                fprintf(WDrun->fout[ch+2], "BoardID: %2d\n", EventInfo->BoardId);
                fprintf(WDrun->fout[ch+2], "Channel: %d\n", ch);
                fprintf(WDrun->fout[ch+2], "Event Number: %d\n", EventInfo->EventCounter);
                fprintf(WDrun->fout[ch+2], "Pattern: 0x%04X\n", EventInfo->Pattern & 0xFFFF);
                fprintf(WDrun->fout[ch+2], "Trigger Time Stamp: %u\n", EventInfo->TriggerTimeTag);
                fprintf(WDrun->fout[ch+2], "DC offset (DAC): 0x%04X\n", WDcfg->DCoffset[ch] & 0xFFFF);
            }
            for(j=0; j<Size; j++) {
                if (WDcfg->Nbit == 8)
                    fprintf(WDrun->fout[ch+2], "%d\n", Event8->DataChannel[ch][j]);
                else
                    fprintf(WDrun->fout[ch+2], "%d\n", Event16->DataChannel[ch][j]);
            }
        }
        if (WDrun->SingleWrite) {
            fclose(WDrun->fout[ch+2]);
            WDrun->fout[ch+2]= NULL;
        }
    }
    return 0;
...
if(elapsed_time == 0){case_n=0;}
else if(0<elapsed_time%20 && elapsed_time%20<10){case_n=1;}
else if(9<elapsed_time%20 && elapsed_time%20<11){case_n=0;}
else if(11<elapsed_time%20){case_n=2;} 
                    ...
                    switch(case_n){
                    	...
                    	case 1:
                    		ret = WriteOutputFiles(&WDcfg, &WDrun, &EventInfo, Event16);
                               ...
                               break;
                       case 2:
                       	ret = WriteOutputFiles1(&WDcfg, &WDrun, &EventInfo, Event16);
                               printf("anti!\n");
                               break;
                               }
